...
def process_file(cur, conn, filepath):
    ...
    df["date_hour"] = pd.to_datetime(pd.to_datetime(df["fired_at"]).dt.strftime("%Y-%m-%d %H:00:00"))
    ...
    # load fact table
    fact_df = pd.DataFrame(index=pd.MultiIndex.from_product([time_idx, df["customer_id"].unique()],
                                                            names=["date_hour", "customer_id"]))

    fact_df["page_loads"] = df[df["event_type"] == "ReferralPageLoad"].groupby(group).size()
    fact_df["clicks"] = df[df["event_type"] == "ReferralRecommendClick"].groupby(group).size()
    fact_df["unique_user_clicks"] = df[df["event_type"] == "ReferralRecommendClick"].groupby(group)["user_id"].nunique()
...
def run():
    ...
    :return:
    """
    conn, cur = create_connection("sample")
